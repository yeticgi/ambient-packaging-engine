...
class Input {
    ...
    getTouchData(finderIndex) {
        if (this._touchData.length > 0) {
            let touchData = find_1(this._touchData, (d) => {
    ...
    _handleTouchMove(event) {
        ...
        for (let i = 0; i < changed.length; i++) {
            let touch = changed.item(i);
            let existingTouch = find_1(this._touchData, d => {
...
class WheelData {
     ...
     */
    getFrame(frame) {
        let wheelFrame = find_1(this._wheelFrames, (f) => {
...
class DeviceCameraQRReader extends DeviceCameraReader {
    ...
    onRenderedToCanvas(canvas) {
        ...
        const code = jsQR$1(imageData.data, imageData.width, imageData.height, {
...
class StateMachine {
    ...
    addStateTransition(fromStateId, command, nextStateId) {
        ...
        const alreadyAdded = this._transitions.some(t => isEqual_1(t, transition));
        if (!alreadyAdded) {
